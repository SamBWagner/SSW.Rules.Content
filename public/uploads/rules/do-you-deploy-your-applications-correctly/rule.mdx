...
related: []
redirects: []
created: 2009-12-04T08:39:40.000Z
archivedreason: null
...
<imageEmbed
  ...
  src="/uploads/rules/do-you-deploy-your-applications-correctly/INSTALLDIR.jpg"
/>
...
<figureEmbed figureEmbed={ {
  preset: "badExample",
  figure: "Bad code - FromFile() method (as well as Process.Start()) give the relative path of the running process. This could mean the path relative to the shortcut or the path relative to the .exe itself, and so an exception will be thrown if the image cannot be found when running from the shortcut",
  shouldDisplay: true
} } />
...
<figureEmbed figureEmbed={ {
  preset: "goodExample",
  figure: "Good code - GetExecutingAssembly().Location will get the pathname of the actual executable and no exception will be thrown",
  shouldDisplay: true
} } />
